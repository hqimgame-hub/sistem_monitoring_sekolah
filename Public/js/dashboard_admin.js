...
import { 
  ...
  serverTimestamp, 
  getDownloadURL
} from "../firebase_init.js"; // Ganti dengan path yang benar
...
async function loadDropdownKelasGlobal() {
  ...
  try {
    ...
    if (snap.empty) {
      tbody.innerHTML = `<tr><td colspan="8" class="p-3 text-gray-500 italic">Belum ada data pengguna</td></tr>`;
      return;
    }
    ...
    <input type="checkbox" class="selectUserCheckbox w-4 h-4" data-id="${docSnap.id}">
  </td>  
      <td class="p-2">${no++}</td>
        ...
        <td class="p-2 text-center space-x-2">
          <button class="px-2 py-1 rounded bg-yellow-500 text-white editUserBtn" data-id="${docSnap.id}">Edit</button>
          <button class="px-2 py-1 rounded bg-red-500 text-white deleteUserBtn" data-id="${docSnap.id}">Hapus</button>
        </td>
      `;
...
async function loadAbsensi() {
  ...
  try {
    ...
    tbody.innerHTML =
      rows || `<tr><td colspan="6" class="text-center text-gray-400 py-3">Tidak ada data sesuai filter</td></tr>`;
...
document.getElementById("btnFilterAbsensi")?.addEventListener("click", () => {
  loadAbsensi();
});
// End of file
// ========================= DASHBOARD ABSENSI =========================

// Pastikan Firebase login aktif agar bisa akses Firestore
try {
  await signInWithEmailAndPassword(auth, "admin@sekolah.com", "admin123");
  console.log("✅ Admin logged in for Dashboard Absensi");
} catch (e) {
  console.warn("⚠️ Gagal login admin otomatis:", e.message);
}
